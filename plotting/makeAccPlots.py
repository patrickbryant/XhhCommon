...
parser.add_option('-c', '--couping',            dest="coupling",              default="1.0", help="")
...
if o.coupling == "1.0":
    ...
    doResolved = False
    import json
    json_data=open(o.signalDir+"/boosted_AccepEffNumbers.json").read()
    data = json.loads(json_data)
    massPts = ["800" ,"900" ,"1000","1100","1200","1300","1400","1500","1600","1800","2000","2250","2500","2750","3000"]    
    print data.keys()
...
if doResolved:
    ...
    cutList.append( cutValue("SR3b",  "3-b Signal Region", ROOT.kRed,     28) )    
    cutList.append( cutValue("SR4b",  "4-b Signal Region", ROOT.kRed,     34) )    
...
for c in cutList:
    ...
    leg.AddEntry(c.graph,c.legName,"P")
    ...
    if not didFirst:
        ...
        if doResolved:
            ...
            c.graph.GetYaxis().SetRangeUser(0,1.4)
...
xatlas, yatlas = 0.3, 0.90
atlas = ROOT.TLatex(xatlas+0.01,   yatlas,      "ATLAS Simulation")
watermarks += [atlas]
#internal = ROOT.TLatex(xatlas+0.17,   yatlas,      "Internal")
internal = ROOT.TLatex(xatlas+0.2,   yatlas-0.005,      "Preliminary")
watermarks += [internal]
if o.coupling in ["1.0","2.0"]:
    ...
    cTag  = ROOT.TLatex(xatlas+0.02,   yatlas-0.06, "Bulk RS, k/#bar{M}_{pl} = 1")
    watermarks += [cTag]
...
if o.coupling == "1.0":
    ...
    savePlot(can, o.output+"/SignalAcc_boostedc10")
...
if   o.coupling == "1.0":
    ...
    outFile = ROOT.TFile(o.output+"/SignalAcc_boostedc10.root","RECREATE")
else:
